...
namespace Thread_.NET.DAL.Context
{
    public static class ModelBuilderExtensions
    {
        ...
        public static void Seed(this ModelBuilder modelBuilder)
        {
            ...
            var postPositiveReactions = GenerateRandomPostReactions(posts, users);
            var postNegativeReactions = GenerateRandomPostNegativeReactions(posts, users);
            var commentPositiveReactions = GenerateRandomCommentReactions(comments, users);
            ...
            modelBuilder.Entity<PostReaction>().HasData(postPositiveReactions);
            modelBuilder.Entity<PostNegativeReaction>().HasData(postNegativeReactions);
            modelBuilder.Entity<CommentReaction>().HasData(commentPositiveReactions);
        ...
        public static ICollection<NegativeReaction> GenerateRandomNegativeReactions(ICollection<User> users)
        {
            int reactionId = 1;

            var reactionsFake = new Faker<NegativeReaction>()
                .RuleFor(r => r.Id, f => reactionId++)
        ...
        {
            int commentPositiveReactionId = 1;

            var commentReactionsFake = new Faker<CommentReaction>()
                .RuleFor(cr => cr.Id, f => commentPositiveReactionId++)
                .RuleFor(cr => cr.IsLike, f => f.Random.Bool())
                .RuleFor(cr => cr.UserId, f => f.PickRandom(users).Id)
        ...
        {
            int commentNegativeReactionId = 1;

            var commentReactionsFake = new Faker<CommentNegativeReaction>()
                .RuleFor(cr => cr.Id, f => commentNegativeReactionId++)
                .RuleFor(cr => cr.IsDislike, f => f.Random.Bool())
                .RuleFor(cr => cr.UserId, f => f.PickRandom(users).Id)
                .RuleFor(cr => cr.CommentId, f => f.PickRandom(comments).Id)
                .RuleFor(pi => pi.CreatedAt, f => DateTime.Now)
